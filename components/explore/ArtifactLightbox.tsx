...
function HtmlLightbox({
  ...
  const maxWidth = typeof window !== "undefined"
    ...
    : 1100;
  const maxHeight = typeof window !== "undefined"
    ? window.innerHeight - BOTTOM_RESERVED - TOP_RESERVED - 52 // 52 = chrome bar + gap
    : 700;
  ...
  return (
    <>
      ...
      >
        ...
        <div style={{ width: 1, height: 16, background: "rgba(255,255,255,0.12)" }} />
        <span className="text-xs text-white/50 shrink-0">{timeAgo(artifact.createdAt)}</span>
      </motion.div>
    </>
  );
}
...
export function ArtifactLightbox({ artifact, onClose, onPrev, onNext, onReact }: ArtifactLightboxProps) {
  ...
  const isHtml  = artifact?.type === "HTML"  && !!artifact.htmlContent;
